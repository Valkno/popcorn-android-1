...
public class YTSProvider extends MediaProvider {
    ...
    public Call getList(final ArrayList<Media> existingList, Filters filters, final Callback callback) {
        ...
        params.add(new NameValuePair("count", "30"));
        ...
        requestBuilder.url(CURRENT_URL + "list?" + query);
        requestBuilder.tag(MEDIA_CALL);
        Timber.d("Connecting to " + CURRENT_URL + "list?" + query);
            ...
            public void onResponse(Response response) throws IOException {
                if (response.isSuccessful()) {
                    ...
                    if(result == null) {
                        ...
                        callback.onFailure(new NetworkErrorException("JSON: " + result.status_message));
                    } else if(result.data != null && ((result.data.get("MovieList") != null && ((ArrayList<LinkedTreeMap<String, Object>>)result.data.get("MovieList")).size() <= 0))) {
                ...
                onFailure(response.request(), new IOException("Couldn't connect to TorrentsAPI"));
            }
        });
    }
    ...
    private class YTSReponse {
        ...
        public ArrayList<Media> formatForPopcorn(ArrayList<Media> existingList) {
            ...
            for (LinkedTreeMap<String, Object> item : movies) {
                ...
                movie.videoId = (String) item.get("imdb");
                ...
                if (existingItem == -1) {
                    ...
                    ArrayList<LinkedTreeMap<String, Object>> torrents =
                            (ArrayList<LinkedTreeMap<String, Object>>) item.get("items");
                    if (torrents != null) {
                        for (LinkedTreeMap<String, Object> torrentObj : torrents) {
                            ...
                            torrent.seeds = ((Double) torrentObj.get("torrent_seeds")).intValue();
                            torrent.peers = ((Double) torrentObj.get("torrent_peers")).intValue();
...
//                            }
                            torrent.url = (String) torrentObj.get("torrent_magnet");
